...
export default props => {
	...
	const [dialogTime, setDialogTime] = useState(
		props.dose.dateTaken
			? new Date(props.dose.dateTaken)
			: new Date(props.dose.date),
	);
	const [dialogAmount, setDialogAmount] = useState(
		props.dose.amount ? '' + props.dose.amount : '1',
	);
	...
	const leftClick = () => {
		if (dose.status !== doseStatus.TOMADA) {
			if (onEdit) {
				...
				setDialogTime(
					originalDose.dateTaken
						? new Date(originalDose.dateTaken)
						: new Date(originalDose.date),
				);
			...
			<>
				<View style={styles.form}>
					...
					<View>
						<TouchableOpacity
							...
							<Text style={onEdit ? styles.hourText : styles.text}>{time}</Text>
						</TouchableOpacity>
					</View>
				...
				<View style={styles.row}>
					...
					<TouchableOpacity
						...
						onPress={rightClick}>
						<View style={[styles.row, styles.centerItems]}>
							<Text style={styles.buttonText}>{rightButtonLabel}</Text>
							<Icon
								name="check-circle"
								type="font-awesome"
								size={20}
