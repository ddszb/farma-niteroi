...
export default props => {
	...
	const options = meds
		.filter(m => m.status == medStatus.ATIVO && !m.scheduledDoses)
	...
	const onConfirm = () => {
		...
		if (taken) {
			...
			if (newStock <= 0) {
				let msg =
					'Seu estoque atual para o medicamento acabou, lembre-se de atualizar o estoque depois. Confirmar dose?';
	...
	const getAmountContent = () => {
		let unit = selectedMed
			? selectedMed.doseUnit.label + '(s)'
			: 'Comprimido(s)';
	...
	const getTimeContent = () => {
		...
		return (
			<>
				<FormFieldLabel>Horário</FormFieldLabel>
				<TouchableOpacity onPress={() => setShowTimePicker(true)}>
					<RowView>
						<HourText>{moment(time).format('HH:mm')}</HourText>
						{taken ? (
							<Icon
								name="check"
								type="font-awesome"
	...
	return (
		<>
			<Form>
				...
				{getAmountContent()}
				<CenteredView>
					<DropdownPicker data={options} onSelect={onSelectMed} />
				</CenteredView>
			...
			<ButtonView>
				<RowView>
					<CancelButton activeOpacity={0.5} onPress={onCancel}>
						<RowView>
