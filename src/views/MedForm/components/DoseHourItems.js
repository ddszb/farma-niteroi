...
export default props => {
	...
	const [doseHoursItems, setDoseHoursItems] = useState([
		{time: defaultDate, amount: 1, unit: props.unit, index: 0},
	]);
	...
	const __updateDoseItem = (event, date) => {
		...
		if (event.type === 'set') {
			...
			var sameTime =
				doses.filter(
					d =>
						d.time.getHours() == date.getHours() &&
						d.time.getMinutes() == date.getMinutes(),
				).length > 0;
			if (sameTime) {
				ToastAndroid.showWithGravityAndOffset(
					'Você já marcou uma dose para este horário.',
					ToastAndroid.SHORT,
					ToastAndroid.BOTTOM,
					0,
					90,
	...
	const __updateDoseTimes = (amountInADay, startHour, startMinute) => {
		...
		if (
			doseHours[doseHours.length - 1].time.getDate() >
			doseHours[0].time.getDate()
		) {
					...
					<View
						...
						<TouchableOpacity
							...
							<View style={{flexDirection: 'row'}}>
								<Text style={style.doseHourText}>{time}</Text>
							</View>
						</TouchableOpacity>
						<TouchableOpacity
							onPress={() => {
							...
							<View>
								<Text style={style.doseHourAmount}>
									{'Tomar ' + d.amount + ' ' + props.unit.label + '(s)'}
								</Text>
							</View>
						</TouchableOpacity>
					</View>
