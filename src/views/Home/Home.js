...
export default props => {
	 ...
	 */
	const filterDoses = () => {
		var allDoses = [].concat.apply(
			[],
			meds.map(m => m.doses),
		); // Concatena todas as doses de todos os medicamentos
		let visible = allDoses.filter(
			d =>
				(moment(d.date).isSame(filterDay, 'day') ||
					moment(d.dateTaken).isSame(filterDay, 'day')) &&
	...
	const onPressDose = dose => {
		if (
			moment().isSame(dose.date, 'day') ||
			moment().isSame(dose.dateTaken, 'day')
		) {
			setEditedDose(dose);
	...
	const updateDose = (dose, action) => {
		var medList = [...meds];
		var updatedMed = medList.filter(
			m => m.id == dose.medId && m.status == medStatus.ATIVO,
		)[0];
		if (updatedMed.doses) {
		...
		updatedMed.doses = updatedMed.doses.map(d =>
			d.index == dose.index ? dose : d,
		);
	...
	return (
		<Container>
			...
			/>
			<TopView>
				<DateFilter date={filterDay} onChangeDate={onChangeDate} />
				<DoseList visibleDoses={visibleDoses} onPressDose={onPressDose} />
			</TopView>
			<ActionButton
				bottom
				label="Nova Dose"
				visible={
					meds.filter(m => m.status == medStatus.ATIVO && !m.scheduledDoses)
						.length > 0 && moment().isSameOrAfter(filterDay, 'date')
				}
